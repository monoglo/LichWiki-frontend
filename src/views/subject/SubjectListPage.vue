            ...
            <v-tabs background-color="gray" class="elevation-2" dark>
              ...
              <v-tab-item>
                <v-card flat tile outlined style="padding: 0px 0px 0px 10px;">
                  <v-card-title class="display-2">大学维基 下的所有学科分类</v-card-title>
                  <v-card-subtitle class="pb-0">大学维基 共有 {{ subject_amount }} 个学科分类。</v-card-subtitle>
                  <v-divider></v-divider>
                  <v-card-text class="text--primary">
                    <v-simple-table fixed-header height="300px">
                      <template v-slot:default>
                        ...
                        <tbody>
                          <tr v-for="item in subject_list" :key="item.s_id">
                            <td>
                              <a @click="goSubjectPage(item.s_name)">{{ item.s_name }}</a>
                            </td>
                          </tr>
                        </tbody>
                      </template>
...
export default {
  ...
  methods: {
    ...
    goSubjectPage: function(subject_name) {
      this.$router.push({
        path:
    ...
    getSubjectList: function() {
      axios
        ...
        .then(res => {
          if (res.data.count > 0) {
            this.subject_amount = res.data.count;
            this.subject_list = res.data.results;
          }
        });
    },
