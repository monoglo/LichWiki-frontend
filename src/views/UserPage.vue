              ...
              <v-tabs background-color="gray" class="elevation-2" dark>
                ...
                <v-tab-item>
                  <v-card flat tile outlined style="padding: 0px 0px 0px 10px;">
                    ...
                    <v-card-subtitle class="pb-0"> 自 {{user.u_register_time}} 以来 {{ user.u_name }} 一共进行了 {{user_article_history.length}} 次更改。</v-card-subtitle>
                    <v-divider></v-divider>
                    ...
                    <v-timeline v-model="user_article_history">
                      ...
                      >
                        ...
                        >
                          <v-card-title class="headline">"{{ item.ah_summary }}" 在词条 "{{ item.article_subject_name }}/{{ item.article_name }}"</v-card-title>
...
export default {
  ...
  methods: {
    ...
    goHistoryPage: function(subject, title, ah_id) {
      this.$router.push({path:'/article/' + subject + '/' + title + '/history/' + ah_id})
    }
  }
};
</script>
